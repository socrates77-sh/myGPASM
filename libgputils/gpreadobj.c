...
_read_opt_header(gp_object_type *object, const unsigned char *file)
{
  ...
  if (object->processor == no_processor)
  {
    ...
    {
    case 8:
      object->processor = gp_find_processor("pic18cxx");
      break;
    case 12:
      object->processor = gp_find_processor("pic16c5x");
      break;
    case 14:
      object->processor = gp_find_processor("pic16cxx");
      break;
    case 16:
      object->processor = gp_find_processor("pic17cxx");
      break;
    }
    if (object->processor == no_processor)
...
_read_sections(gp_object_type *object, const unsigned char *file)
{
  ...
  section_ptr = file + (object->isnew ? (FILE_HDR_SIZ_v2 + OPT_HDR_SIZ_v2) : (FILE_HDR_SIZ_v1 + OPT_HDR_SIZ_v1));

  /* setup pointer to string table */
  string_table = (const char *)&file[object->symbol_ptr +
