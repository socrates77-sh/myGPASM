...
static void end_record_s19()
{
  write_byte(0xff - (sum & 0xff));
  fprintf(hex, "%s", newline);
}
...
void write_i_mem_s19(unsigned int core_size, pic_processor_t processor)
{
  MemBlock *m = memory;
  int i, j, maximum;
...
void write_i_mem_s19(unsigned int core_size)
{
  ...
  while(m) {
    ...
    maximum = 2 * (core_size+1);

	for(i=0; i<maximum/0x20; i++)
		data_line_s19(0x20*i, 0x20*(i+1));
