...
void before_exit_failure()
{
  char s19filename[BUFSIZ];
  char lstfilename[BUFSIZ];
  printf("\n       ===== (GPLINK) Unuccessful =====\n\n");
  snprintf(s19filename, sizeof(s19filename), "%s.s19", state.basefilename);
  snprintf(lstfilename, sizeof(lstfilename), "%s.lst", state.basefilename);
  unlink(s19filename);
...
*/
static void parse_define(const char *optarg,
