                          ...
                          struct pnode *parms)
{
  ...
  case 2:
    ...
    if (ca != state.processor->config_addrs[0] && ca != state.processor->config_addrs[1])
      gperror(GPE_CONFIG_UNKNOWN, "__CONFIG address is wrong");
    p = HEAD(TAIL(parms));
    break;
                          ...
                          struct pnode *parms)
{
  ...
  if (p->tag == string)
  {
    ...
    {
      if ((get_symbol(state.stDefines, p->value.string) != NULL) && (state.pass == 1))
      {
        /* FIXME: find a more elegant way to do this */
        state.pass = 2;
    ...
    if (asm_enabled() || (i->attribs & ATTRIB_COND))
    {
      ...
      case INSN_CLASS_LIT10:
        if (enforce_arity(arity, 1))
        {
          ...
          /* PC is 10 bits.  mpasm checks the maximum device address. */
          if (value & (~0x3ff))
            gperror(GPE_RANGE, NULL);

          emit(i->opcode | (value & 0x3ff));
        }
        break;
      ...
      case INSN_CLASS_LIT12:
        if (enforce_arity(arity, 1))
        {
          ...
          if ((value & 0xf000) != (r & 0xf000))
            gpmessage(GPM_PAGE, NULL);

          emit(i->opcode | (value & 0xfff));
        }
        break;
      ...
      case INSN_CLASS_LIT16:
        if (enforce_arity(arity, 1))
        {
          int dest;
          p = HEAD(parms);
          dest = reloc_evaluate(p, RELOCT_LGOTO);
          dest = gp_processor_org_to_byte(state.device.class, dest) >> 1;
          emit(i->opcode | (dest >> 8 & 0xff));
          //emit(i->opcode);
          reloc_evaluate(p, RELOCT_LGOTO2); /* add the second relocation */
          emit_check(0x2300, (dest & 0xff), 0xff);
          //emit_check(0x0000, dest, state.processor->maxrom);
        }
        break;

      case INSN_CLASS_LIT11L:
        if (enforce_arity(arity, 1))
        {
          int dest;
          p = HEAD(parms);
          dest = reloc_evaluate(p, RELOCT_LGOTO);
          dest = gp_processor_org_to_byte(state.device.class, dest) >> 1;
          emit(i->opcode | (dest >> 5 & 0x3f));
          //emit(i->opcode);
          reloc_evaluate(p, RELOCT_LGOTO2); /* add the second relocation */
          emit_check(0x0020, (dest & 0x1f), 0xff);
          //emit_check(0x0000, dest, state.processor->maxrom);
        }
        break;
      ...
      case INSN_CLASS_OPF7:
        if (enforce_arity(arity, 1))
        {
          ...
          /* 
          else if (strcasecmp(i->name, "iost") == 0) {
            file = reloc_evaluate(p, RELOCT_TRIS);
			if(file>7 || file<0)
				gperror(GPE_ILLEGAL_ARGU, NULL);
          } */
      ...
      case INSN_CLASS_OPF6:
        if (enforce_arity(arity, 1))
        {
          ...
          file_ok(file);
          emit(i->opcode | (file & 0x3f));
        }
        break;

      case INSN_CLASS_OPF9:
        if (enforce_arity(arity, 1))
        {
          p = HEAD(parms);
          file = reloc_evaluate(p, RELOCT_F);
          file_ok(file);
          emit(i->opcode | (file & 0x1ff));
        }
        break;
      ...
      case INSN_CLASS_B6:
      {
        ...
        if (enforce_arity(arity, 2))
        {
          ...
          if (!((0 <= bit) && (bit <= 7)))
            gpwarning(GPW_RANGE, NULL);
          file_ok(file);
          emit(i->opcode | ((bit & 7) << 6) | (file & 0x3f));
        }
      }
      break;
      ...
      case INSN_CLASS_B9:
      {
        ...
        if (enforce_arity(arity, 2))
        {
          ...
          if (!((0 <= bit) && (bit <= 7)))
            gpwarning(GPW_RANGE, NULL);
          file_ok(file);
          emit(i->opcode | ((bit & 7) << 9) | (file & 0x1ff));
        }
      }
      break;
...
void opcode_init(int stage)
{
  ...
  for (i = 0; i < count; i++)
  {
    ...
    /*	if(strcmp(base[i].name, "lgoto")==0)
	{
		struct insn *t;
		t = get_symbol_annotation(get_symbol(state.stBuiltin, "lgoto"));
		printf("test\n");
	}*/
  }
